...
public class QuestionVersion {
    ...
    private void apply(Content c, Instant now) {
        ...
        if (questionType == QuestionType.CODING) {
            requireCoding(c);
            constraintsText=c.constraintsText(); examples=c.examples(); supportedLanguages=c.supportedLanguages(); programmingLanguages=c.supportedLanguages(); visibleTests=c.visibleTests(); hiddenTests=c.hiddenTests(); scoringRules=c.scoringRules(); executionLimits=c.executionLimits();
            defaultScoringPolicy=JsonNodeFactory.instance.objectNode().put("policyCode", "TEST_CASES").set("parameters", c.scoringRules());
            options=List.of(); correctOptionId=null; explanation=null;
        } else {
            validateMcq(c);
            constraintsText=null; examples=null; supportedLanguages=null; programmingLanguages=JsonNodeFactory.instance.arrayNode(); visibleTests=null; hiddenTests=null; scoringRules=null; executionLimits=null;
    ...
    private static void requireCoding(Content c) {
        if (c.constraintsText() == null || c.constraintsText().isBlank() || c.examples() == null || c.supportedLanguages() == null || c.visibleTests() == null || c.hiddenTests() == null || c.scoringRules() == null || c.executionLimits() == null) throw new IllegalArgumentException("Coding questions require coding content");
