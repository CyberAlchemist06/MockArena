...
public class QuestionVersion {
    ...
    private void apply(Content c, Instant now) { title=c.title(); tags=normalizeTags(c.tags()); difficulty=c.difficulty(); prompt=c.prompt(); constraintsText=c.constraintsText(); examples=c.examples(); supportedLanguages=c.supportedLanguages(); visibleTests=c.visibleTests(); hiddenTests=c.hiddenTests(); scoringRules=c.scoringRules(); executionLimits=c.executionLimits(); updatedAt=now; }
    ...
    public record Content(String title, List<String> tags, Difficulty difficulty, String prompt, String constraintsText, JsonNode examples, JsonNode supportedLanguages, JsonNode visibleTests, JsonNode hiddenTests, JsonNode scoringRules, JsonNode executionLimits) { }
}
