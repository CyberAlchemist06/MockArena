...
public class QuestionController {
    private final QuestionApplicationService service;
    public QuestionController(QuestionApplicationService service) { this.service=service; }
    @PostMapping public ResponseEntity<QuestionVersionResponse> create(@Valid @RequestBody CreateQuestionRequest request) {
        QuestionVersionResponse response=QuestionDtos.toResponse(service.create(request));
    ...
    @PostMapping("/{questionId}/versions") @ResponseStatus(HttpStatus.CREATED) public QuestionVersionResponse revise(@PathVariable UUID questionId,@RequestParam long expectedQuestionVersion) { return QuestionDtos.toResponse(service.createRevision(questionId,expectedQuestionVersion)); }
