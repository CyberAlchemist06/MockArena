...
public interface CodingEvaluationJobRepository extends JpaRepository<CodingEvaluationJob,UUID>{
 ...
 @Query(value="select * from evaluation.coding_evaluation_jobs where (status='RECEIVED' and next_attempt_at <= :now) or (status='RETRY_WAIT' and result_outcome is null and next_attempt_at <= :now) or (status='RESOLVING_SPEC' and lease_expires_at < :now) order by created_at for update skip locked limit :limit",nativeQuery=true) List<CodingEvaluationJob> claimable(@Param("now") Instant now,@Param("limit") int limit);
 @Query(value="select * from evaluation.coding_evaluation_jobs where (status='READY_FOR_RESULT_APPLICATION') or (status='RETRY_WAIT' and result_outcome is not null and next_attempt_at <= :now) order by created_at limit :limit",nativeQuery=true) List<CodingEvaluationJob> deliveryReady(@Param("now") Instant now,@Param("limit") int limit);
