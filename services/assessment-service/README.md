...
Coding evaluation Phase 2 relays V9 outbox entries to Evaluation Service with PostgreSQL claim leases. The relay is disabled by default (`ASSESSMENT_CODING_EVALUATION_RELAY_ENABLED=false`) and requires an externally supplied Evaluation workload token when enabled. The internal coding snapshot endpoint is workload-only and is not a browser API.
