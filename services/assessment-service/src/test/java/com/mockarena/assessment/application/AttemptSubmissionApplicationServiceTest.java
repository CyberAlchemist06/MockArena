...
import com.mockarena.assessment.domain.*; import org.junit.jupiter.api.Test; import org.springframework.context.ApplicationEventPublisher; import java.time.Instant; import java.util.*; import static org.assertj.core.api.Assertions.*; import static org.mockito.Mockito.*;
class AttemptSubmissionApplicationServiceTest {
 @Test void serializes_submit_replay_and_later_response_locking_without_remote_clients(){
  ...
  var first=service.submit(id,user,"key");assertThat(first.status()).isEqualTo("SUBMITTED");assertThat(first.submittedAt()).isNotNull();verify(requests).saveAndFlush(any(AttemptSubmitRequest.class));verify(events).publishEvent(new AttemptSubmittedEvent(id));
