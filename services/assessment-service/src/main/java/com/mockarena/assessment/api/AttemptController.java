...
@RestController @RequestMapping("/api/v1/attempts") public class AttemptController {
    private final AttemptContentApplicationService content; private final AttemptResponseApplicationService responses; private final AttemptSubmissionApplicationService submissions; private final AttemptResultReadApplicationService results; private final CurrentAuthenticatedUser current;
    public AttemptController(AttemptContentApplicationService content, AttemptResponseApplicationService responses, AttemptSubmissionApplicationService submissions, AttemptResultReadApplicationService results, CurrentAuthenticatedUser current){this.content=content;this.responses=responses;this.submissions=submissions;this.results=results;this.current=current;}
    ...
    @GetMapping("/{attemptId}/result") public CandidateAttemptResultResponse result(@PathVariable UUID attemptId){return results.result(attemptId,current.userId());}
}
